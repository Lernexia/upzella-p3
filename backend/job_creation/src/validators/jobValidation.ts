...
const salaryDetailsSchema = Joi.object({
  ...
  salary_from: Joi.any().optional(),
  salary_to: Joi.any().optional(),
  salary_period: Joi.string()
    .valid("per hour", "per month", "per annum")
    .optional(),
})
  .custom((value, helpers) => {
    if (
      value.salary_from &&
      value.salary_to &&
      value.salary_from > value.salary_to
    ) {
...
const experienceDetailsSchema = Joi.object({
  experience_min: Joi.number().integer().min(0).optional().default(0),
  experience_max: Joi.number().integer().min(0).optional().default(5),
})
  .custom((value, helpers) => {
    if (value.experience_min !== undefined && value.experience_max !== undefined && value.experience_min > value.experience_max) {
      return helpers.error("any.invalid");
    }
    return value;
...
const resumeScoreWeightageSchema = Joi.array()
  .items(
    Joi.object({
      resume_section: Joi.string().required(),
      resume_criteria: Joi.string().min(10).max(500).required(),
      resume_weightage: Joi.number().integer().min(1).max(100).required(),
      reason: Joi.string().min(20).max(300).optional(),
    })
  )
  .custom((value, helpers) => {
    // Only validate total weightage if there are items
    if (!value || value.length === 0) return value;
    
    // Check if weightage sums to 100
    const totalWeightage = value.reduce(
      (sum: number, item: any) => sum + item.resume_weightage,
      0
    );
    ...
    return value;
  }, "Resume weightage sum validation")
  .optional(); // Make it optional for drafts

// Work type validation
const workTypeSchema = Joi.array()
  .items(Joi.string().valid("remote", "hybrid", "onsite", "all"))
  .optional();

// Employment type validation
const employmentTypeSchema = Joi.array()
  .items(
    Joi.string().valid(
      "part-time",
      "full-time",
...
const seniorityLevelSchema = Joi.array()
  .items(
    Joi.string().valid(
      "entry_level",
      "fresher",
...
// Compensation validation - matches database ARRAY field
const compensationSchema = Joi.array()
  .items(Joi.string())
  .optional();
...
export const createJobSchema = Joi.object({
  ...
  description: Joi.string().min(10).max(5000).required(), // Reduced min for drafts
  ...
  experience_details: experienceDetailsSchema.optional(), // Make optional for drafts
